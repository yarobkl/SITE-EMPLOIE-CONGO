...
export default function MessagingCenter() {
  ...
  const loadApplications = useCallback(async () => {
    ...
    setApplications((data || []).filter((item) =>
      item.candidate_id === user.id || item.jobs?.companies?.owner_id === user.id,
    ));
  }, [user]);
          ...
          <div className="border-b border-slate-200 p-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h2 className="mt-1 text-2xl font-bold">Messages</h2>
              </div>
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="flex justify-between gap-2">
                      ...
                      <small className="shrink-0 text-slate-500">{dateLabel(thread.last_message_at)}</small>
                    </span>
          ...
          {selected ? (
            <>
              <header className="flex min-h-[82px] items-center gap-3 border-b border-slate-200 bg-white px-4 md:px-6">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="truncate font-bold">{partner}</h3>
              ...
              <div className="flex-1 overflow-y-auto px-4 py-6 md:px-8">
                <div className="mx-auto max-w-3xl space-y-4">
                      ...
                      <div key={message.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[82%] rounded-2xl px-4 py-3 shadow-sm ${mine ? 'rounded-br-md bg-blue-700 text-white' : 'rounded-bl-md border border-slate-200 bg-white'}`}>
                          ...
                          <p className={`mt-1 text-right text-[10px] ${mine ? 'text-blue-100' : 'text-slate-400'}`}>
                            {dateLabel(message.created_at)}{mine && message.read_at ? ' · Lu' : ''}
                          </p>
                        </div>
                      </div>
                    );
                  ...
                  {messages.length === 0 && (
                    <p className="py-12 text-center text-sm text-slate-500">Écrivez le premier message.</p>
                  )}
                </div>
              </div>

              <form onSubmit={send} className="border-t border-slate-200 bg-white p-3 md:p-5">
                <div className="mx-auto flex max-w-3xl items-end gap-2">
                  ...
                  />
                  <button type="submit" disabled={!draft.trim() || busy} className="primary-button h-12 w-12 px-0" aria-label="Envoyer">
                    <Send size={19} />
                  </button>
                </div>
            ...
            <div className="flex flex-1 items-center justify-center p-8 text-center">
              <div>
                ...
                <h3 className="mt-4 text-xl font-bold">Vos échanges de recrutement</h3>
