...
const LEGAL_PAGES = {
  privacy: {
    ...
    sections: [
      ...
      ['Sécurité', 'Contrôle d’accès par rôle, politiques de sécurité au niveau des lignes de base de données, bucket CV privé, liens temporaires, limitation des types et de la taille des fichiers, journalisation fonctionnelle des consultations et séparation des accès candidats, recruteurs et administrateurs.'],
  ...
  rights: {
    ...
    intro: 'Déposez une demande et conservez la référence qui sera générée.',
    sections: [],
  },
};
...
function createConsentBlock(form, kind, openLegal) {
  ...
  const checkboxId = `nzela-${kind}-consent`;
  block.innerHTML = `
    <label style="display:flex;align-items:flex-start;gap:9px;cursor:pointer">
      <input id="${checkboxId}" type="checkbox" data-nzela-required-consent style="margin-top:3px" />
      <span>${kind === 'signup'
...
export default function PrivacyComplianceExperience() {
    ...
    const records = [
      {
        ...
        source: 'web_signup',
        granted_at: pending.grantedAt || now,
      },
      {
        user_id: user.id,
        subject_email: user.email,
  ...
  useEffect(() => {
    ...
    const apply = () => {
      ...
      frame = requestAnimationFrame(() => {
        const signupHeading = Array.from(document.querySelectorAll('h1,h2')).find((heading) =>
          /créer votre compte|inscription/i.test(textOf(heading)),
        );
        ...
        const applicationButton = Array.from(document.querySelectorAll('button')).find((button) =>
          /envoyer.*candidature|postuler maintenant/i.test(textOf(button)),
        );
        createConsentBlock(applicationButton?.closest('form'), 'application', openLegal);

        const bodyText = document.body?.innerText || '';
        const reference = bodyText.match(/Candidature envoy[eé]e\.\s*R[eé]f[eé]rence\s+(NZJ-[A-Z0-9-]+)/i)?.[1];
        const pending = JSON.parse(localStorage.getItem(PENDING_APPLICATION_KEY) || 'null');
  ...
  const exportMyData = async () => {
    if (!session?.user || !supabase) {
      setMessage('Connectez-vous pour exporter les données liées à votre compte.');
      return;
    }
    ...
    link.href = url;
    link.download = `nzela-jobs-donnees-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  const submitPrivacyRequest = async (event) => {
    ...
    if (!hasSupabaseConfig || !supabase) {
      setMessage('Le service de demande est temporairement indisponible.');
      return;
    }
    const email = requestForm.email.trim();
    if (!email) {
      setMessage('Indiquez votre adresse électronique.');
      return;
    }
    const { error } = await supabase.from('privacy_requests').insert({
      user_id: session?.user?.id || null,
      email,
      tracking_number: requestForm.tracking.trim() || null,
      request_type: requestForm.type,
      details: requestForm.details.trim() || null,
    });
    if (error) {
      setMessage(`Demande non enregistrée : ${error.message}`);
      return;
    }
            ...
            <main style={styles.content}>
              ...
              {currentPage.sections.map(([title, body]) => (
                <section key={title} style={{ marginTop: 24 }}>
                  <h3 style={{ color: '#0f172a', marginBottom: 6 }}>{title}</h3>
              ...
              {page === 'rights' && (
                <section style={{ marginTop: 18 }}>
                  {session?.user && (
                    <button type="button" onClick={exportMyData} style={{ ...styles.tab, background: '#0f172a', color: '#fff', borderColor: '#0f172a', marginBottom: 18 }}>Télécharger mes données</button>
                  )}
                  <form onSubmit={submitPrivacyRequest} style={{ display: 'grid', gap: 12, maxWidth: 680 }}>
                    <label>
                      <span style={{ display: 'block', fontWeight: 800, marginBottom: 5 }}>Adresse électronique</span>
                      <input type="email" required value={requestForm.email} onChange={(event) => setRequestForm((current) => ({ ...current, email: event.target.value }))} style={{ width: '100%', padding: 12, border: '1px solid #cbd5e1', borderRadius: 10 }} />
                    </label>
                    <label>
                      <span style={{ display: 'block', fontWeight: 800, marginBottom: 5 }}>Référence de candidature, le cas échéant</span>
                      <input value={requestForm.tracking} onChange={(event) => setRequestForm((current) => ({ ...current, tracking: event.target.value }))} placeholder="NZJ-CAND-..." style={{ width: '100%', padding: 12, border: '1px solid #cbd5e1', borderRadius: 10 }} />
                    </label>
                    <label>
                      <span style={{ display: 'block', fontWeight: 800, marginBottom: 5 }}>Type de demande</span>
                      <select value={requestForm.type} onChange={(event) => setRequestForm((current) => ({ ...current, type: event.target.value }))} style={{ width: '100%', padding: 12, border: '1px solid #cbd5e1', borderRadius: 10 }}>
                        ...
                        <option value="consent_withdrawal">Retrait du consentement</option>
                      </select>
                    </label>
                    <label>
                      <span style={{ display: 'block', fontWeight: 800, marginBottom: 5 }}>Précisions</span>
                      <textarea value={requestForm.details} onChange={(event) => setRequestForm((current) => ({ ...current, details: event.target.value }))} rows={4} style={{ width: '100%', padding: 12, border: '1px solid #cbd5e1', borderRadius: 10, resize: 'vertical' }} />
                    </label>
