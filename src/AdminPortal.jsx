...
function StatusBadge({ status }) {
  ...
  return <span className={cx('inline-flex rounded-full px-2.5 py-1 text-[11px] font-black', tone[status] || 'bg-slate-100 text-slate-700')}>{statusLabel(status)}</span>;
}
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
      <div className="flex items-start justify-between gap-4">
        <div><p className="text-sm font-semibold text-slate-500">{label}</p><p className="mt-2 text-3xl font-black tracking-tight text-slate-950">{value}</p></div>
        <div className={cx('flex h-11 w-11 items-center justify-center rounded-xl', iconTone[tone] || iconTone.blue)}><Icon size={20} /></div>
      </div>
...
function EmptyState({ title, body }) {
  return <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50/60 px-5 py-9 text-center"><ClipboardList className="mx-auto text-slate-300" size={32} /><p className="mt-3 font-black text-slate-800">{title}</p><p className="mt-1 text-sm text-slate-500">{body}</p></div>;
}
...
function LoginScreen({ onSession }) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#f6f7f9] px-4 py-10">
      <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-7 shadow-xl shadow-slate-200/50 sm:p-8">
        <div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-slate-950 text-white"><ShieldCheck size={21} /></div><div><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">Nzela Jobs</p><h1 className="text-xl font-black text-slate-950">Administration</h1></div></div>
        <p className="mt-6 text-sm leading-6 text-slate-500">Espace indépendant réservé au pilotage, aux vérifications et à la modération de Nzela Jobs.</p>
        {!hasSupabaseConfig && <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm font-semibold text-amber-900">Supabase n’est pas configuré pour ce déploiement.</div>}
        ...
        <form onSubmit={signIn} className="mt-6 space-y-4">
          <label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-600">Adresse e-mail</span><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="h-12 w-full rounded-xl border border-slate-300 px-4 text-sm outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-50" /></label>
          <label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-600">Mot de passe</span><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="h-12 w-full rounded-xl border border-slate-300 px-4 text-sm outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-50" /></label>
          <button disabled={busy || !hasSupabaseConfig} className="h-12 w-full rounded-xl bg-slate-950 px-4 text-sm font-black text-white disabled:opacity-50">{busy ? 'Connexion…' : 'Se connecter'}</button>
        </form>
        ...
        <button type="button" onClick={google} disabled={busy || !hasSupabaseConfig} className="h-12 w-full rounded-xl border border-slate-300 bg-white px-4 text-sm font-black text-slate-700 disabled:opacity-50">Continuer avec Google</button>
      </div>
    </div>
  );
}
...
export default function AdminPortal() {
  ...
  const resolveAccess = useCallback(async (nextSession) => {
    ...
    if (!nextSession?.user || !supabase) return setAccess('guest');
    if (nextSession.user.email?.toLowerCase() === PRIMARY_EMAIL) return setAccess('admin');
    const { data, error: roleError } = await supabase.from('profiles').select('role').eq('id', nextSession.user.id).maybeSingle();
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    if (results.some((result) => result.error)) setError('Certaines données administratives ne peuvent pas être chargées pour le moment.');
  ...
  useEffect(() => {
    ...
    load();
    const channel = supabase.channel('nzela-admin-live')
  ...
  if (access === 'denied') return <div className="flex min-h-screen items-center justify-center bg-[#f6f7f9] px-4"><div className="max-w-md rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-xl"><ShieldAlert className="mx-auto text-red-500" size={42} /><h1 className="mt-4 text-xl font-black">Accès non autorisé</h1><p className="mt-2 text-sm text-slate-500">Ce compte n’a pas de rôle administrateur Nzela Jobs.</p><button type="button" onClick={signOut} className="mt-6 h-11 rounded-xl bg-slate-950 px-5 text-sm font-black text-white">Se déconnecter</button></div></div>;

  return (
    <div className="min-h-screen bg-[#f6f7f9] text-slate-950">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[268px] border-r border-slate-200 bg-white lg:block">
        <div className="flex h-20 items-center gap-3 border-b border-slate-100 px-5"><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-950 text-white"><ShieldCheck size={20} /></div><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-blue-700">Nzela Jobs</p><p className="text-base font-black">Admin</p></div></div>
        <nav className="space-y-1 p-4">{NAV.map(([id, label, Icon]) => {
          const badge = id === 'verifications' ? pendingVerifications : id === 'moderation' ? pendingModeration : 0;
          return <button key={id} type="button" onClick={() => setTab(id)} className={cx('flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-left text-sm font-bold transition', tab === id ? 'bg-slate-950 text-white' : 'text-slate-600 hover:bg-slate-100')}><Icon size={18} /><span className="flex-1">{label}</span>{badge > 0 && <span className={cx('rounded-full px-2 py-0.5 text-[10px] font-black', tab === id ? 'bg-white text-slate-950' : 'bg-red-50 text-red-700')}>{badge}</span>}</button>;
        })}</nav>
        <div className="absolute bottom-0 left-0 right-0 border-t border-slate-100 p-4"><div className="rounded-2xl bg-slate-50 p-3"><p className="truncate text-xs font-black text-slate-800">{session?.user?.email}</p><p className="mt-1 text-[11px] font-semibold text-slate-400">Administrateur Nzela</p><button type="button" onClick={signOut} className="mt-3 flex min-h-10 w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white text-xs font-black text-slate-700"><LogOut size={15} /> Déconnexion</button></div></div>
      </aside>

      <div className="lg:pl-[268px]">
        <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
          <div className="flex min-h-20 items-center gap-3 px-4 sm:px-6 xl:px-8"><div className="min-w-0 flex-1"><p className="text-xs font-bold text-slate-400">Nzela Jobs / Administration</p><h1 className="truncate text-xl font-black tracking-tight">{NAV.find(([id]) => id === tab)?.[1]}</h1></div><div className="hidden w-full max-w-sm items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 md:flex"><Search size={17} className="text-slate-400" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Rechercher…" className="h-11 min-w-0 flex-1 bg-transparent text-sm outline-none" />{query && <button type="button" onClick={() => setQuery('')}><X size={16} className="text-slate-400" /></button>}</div><button type="button" onClick={() => load()} disabled={loading} className="flex h-11 w-11 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 disabled:opacity-50"><RefreshCw size={18} className={loading ? 'animate-spin' : ''} /></button></div>
        ...
        <main className="mx-auto max-w-[1500px] space-y-6 p-4 sm:p-6 xl:p-8">
          ...
          {tab === 'overview' && <>
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Utilisateurs" value={snapshot.users_total} note={`${snapshot.candidates_total} candidats · ${snapshot.recruiters_total} recruteurs`} icon={User} /><MetricCard label="Candidatures" value={snapshot.applications_total} note={`${snapshot.applications_today} reçue(s) aujourd’hui`} icon={ClipboardList} tone="emerald" /><MetricCard label="Offres d’emploi" value={snapshot.jobs_total} note={`${snapshot.jobs_published} en ligne`} icon={Briefcase} tone="violet" /><MetricCard label="Entreprises" value={snapshot.companies_total} note={`${marketplace.verified_companies} vérifiée(s)`} icon={Building2} tone="amber" /></div>
            <div className="grid gap-6 xl:grid-cols-[1.45fr_0.85fr]"><Panel title="Activité de la plateforme" eyebrow="7 derniers jours"><div className="mt-6 flex h-64 items-end gap-3 sm:gap-5">{activitySeries.map((item) => <div key={item.key} className="flex min-w-0 flex-1 flex-col items-center justify-end gap-2"><span className="text-xs font-black text-slate-500">{item.value}</span><div className="flex h-44 w-full items-end rounded-xl bg-slate-50 p-1"><div className="w-full rounded-lg bg-slate-950" style={{ height: `${Math.max(6, (item.value / maxActivity) * 100)}%` }} /></div><span className="text-[11px] font-bold capitalize text-slate-400">{item.label}</span></div>)}</div></Panel><Panel title="État aujourd’hui" eyebrow="Temps réel"><div className="mt-5 space-y-3">{[['Connectés maintenant', snapshot.online_now, Eye], ['Actifs aujourd’hui', snapshot.active_today, Activity], ['Messages non lus', snapshot.messages_unread, Bell], ['Offres favorites', snapshot.favorites_total, Briefcase]].map(([label, value, Icon]) => <div key={label} className="flex items-center gap-3 rounded-xl bg-slate-50 p-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white text-slate-700 shadow-sm"><Icon size={18} /></div><p className="flex-1 text-sm font-bold text-slate-600">{label}</p><p className="text-lg font-black">{value}</p></div>)}</div></Panel></div>
            <div className="grid gap-6 xl:grid-cols-[1.25fr_0.95fr]"><Panel title="Offres les plus performantes" eyebrow="Candidatures"><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[620px] text-left"><thead><tr className="border-b border-slate-100 text-[11px] font-black uppercase text-slate-400"><th className="pb-3">Offre</th><th className="pb-3">Statut</th><th className="pb-3 text-right">Candidatures</th><th className="pb-3 text-right">Vues</th></tr></thead><tbody>{topJobs.map((job) => <tr key={job.job_id} className="border-b border-slate-100 last:border-0"><td className="py-4"><p className="font-black">{job.title}</p><p className="mt-1 text-xs font-semibold text-slate-400">{job.company_name} · {job.location}</p></td><td><StatusBadge status={job.status} /></td><td className="text-right font-black">{job.application_count}</td><td className="text-right font-bold text-slate-500">{job.view_count}</td></tr>)}</tbody></table></div></Panel><Panel title="Activité récente" eyebrow="Flux live"><div className="mt-4 space-y-1">{activity.slice(0, 7).map((item, index) => <div key={`${item.entity_id}-${index}`} className="flex gap-3 rounded-xl p-3 hover:bg-slate-50"><span className="mt-1 h-2.5 w-2.5 shrink-0 rounded-full bg-blue-600" /><div className="min-w-0 flex-1"><p className="text-sm font-black">{ACTIVITY_LABELS[item.event_type] || 'Activité plateforme'}</p><p className="truncate text-xs font-semibold text-slate-500">{item.actor} · {item.detail}</p></div><p className="shrink-0 text-[10px] font-bold text-slate-400">{formatDate(item.created_at, true)}</p></div>)}{activity.length === 0 && <EmptyState title="Aucune activité" body="Les nouvelles actions apparaîtront ici." />}</div></Panel></div>
          </>}

          {tab === 'marketplace' && <><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Couverture des offres" value={`${marketplace.job_application_coverage_pct}%`} note={`${marketplace.live_jobs_without_applications} offre(s) sans candidature`} icon={Briefcase} /><MetricCard label="Candidatures / offre" value={marketplace.avg_applications_per_live_job} note="Moyenne des offres en ligne" icon={ClipboardList} tone="emerald" /><MetricCard label="Vers conversation" value={`${marketplace.application_to_conversation_pct}%`} note={`${marketplace.applications_with_conversation} conversation(s)`} icon={Bell} tone="violet" /><MetricCard label="Entreprises vérifiées" value={`${marketplace.verified_company_pct}%`} note={`${marketplace.verified_companies}/${marketplace.companies_total}`} icon={ShieldCheck} tone="amber" /></div><Panel title="Offres à surveiller" eyebrow="Action requise"><div className="mt-5 grid gap-3 lg:grid-cols-2">{atRisk.map((item) => <article key={item.job_id} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase text-slate-400">{item.company_name}</p><h3 className="mt-1 font-black">{item.title}</h3></div><span className={cx('rounded-full px-2.5 py-1 text-[11px] font-black', item.health === 'critical' ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-800')}>{item.health === 'critical' ? 'Critique' : 'À surveiller'}</span></div><p className="mt-3 text-sm font-semibold text-slate-500">0 candidature · {Math.max(1, Math.floor(Number(item.age_hours || 0) / 24))} jour(s) en ligne</p></article>)}{atRisk.length === 0 && <div className="lg:col-span-2"><EmptyState title="Aucune offre à risque" body="Toutes les offres ont reçu une candidature." /></div>}</div></Panel></>}
