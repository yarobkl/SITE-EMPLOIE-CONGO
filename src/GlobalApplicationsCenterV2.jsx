      ...
      <div className="mx-auto flex h-full max-w-[1380px] flex-col overflow-hidden bg-white shadow-2xl md:h-[calc(100vh-3rem)] md:rounded-2xl md:border md:border-slate-200">
        <header className="border-b border-slate-200 p-4 md:p-6">
          <div className="flex items-start justify-between gap-4">
            <div><p className="text-xs font-bold uppercase tracking-[0.14em] text-blue-700">Recruteur principal</p><h2 className="mt-1 text-2xl font-bold text-slate-950 md:text-3xl">Toutes les candidatures</h2><p className="mt-1 text-sm text-slate-500">Dossiers suivis et rapides, anciens et nouveaux.</p></div>
          ...
          <div className="mt-5 grid gap-2 md:grid-cols-[1fr_190px_auto]">
            <label className="flex min-h-11 items-center gap-3 rounded-lg border border-slate-300 px-3 focus-within:border-blue-700 focus-within:ring-2 focus-within:ring-blue-600"><Search size={18} className="text-slate-500" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Nom, email, offre, entreprise…" className="min-w-0 flex-1 bg-transparent text-sm outline-none" /></label>
              ...
              <article key={item.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:p-5">
                <div className="flex flex-col gap-4 lg:flex-row lg:justify-between">
                  <div className="min-w-0">
                    <div className="flex flex-wrap gap-2"><span className={`rounded-full px-3 py-1 text-xs font-bold ${STATUS_CLASS[item.status]}`}>{STATUSES.find(([value]) => value === item.status)?.[1] || item.status}</span>{today(item.createdAt) && <span className="rounded-full bg-violet-50 px-3 py-1 text-xs font-bold text-violet-700">Aujourd’hui</span>}<span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">{item.tracked ? 'Suivie' : 'Rapide'}</span></div>
                    <h3 className="mt-3 text-lg font-bold text-slate-950">{item.name}</h3><p className="mt-1 text-sm font-semibold text-blue-700">{item.jobTitle}</p><p className="mt-1 flex items-center gap-2 text-sm text-slate-600"><Building2 size={16} />{item.companyName}</p><div className="mt-3 grid gap-1 text-sm text-slate-600 sm:grid-cols-2"><span>{item.email || 'Email non renseigné'}</span><span>{item.phone || 'Téléphone non renseigné'}</span></div>
                  </div>
