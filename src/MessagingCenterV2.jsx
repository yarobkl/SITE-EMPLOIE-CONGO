...
const initials = (value = 'Nzela Jobs') => value
  .split(/\s+/)
  .filter(Boolean)
  .slice(0, 2)
  .map((part) => part[0])
...
function openMainLogin() {
  ...
  const timer = window.setInterval(() => {
    ...
    const loginButton = Array.from(document.querySelectorAll('main button')).find((button) =>
      /^Connexion(?: candidat)?$/i.test(textOf(button)),
    );
...
export default function MessagingCenterV2() {
  ...
  const loadThreads = useCallback(async (currentUser = user) => {
    ...
    if (threadError) {
      setError('Impossible de charger votre messagerie.');
      return;
    }
  ...
  const loadApplications = useCallback(async (currentUser = user) => {
    ...
    setApplications((data || []).filter((item) =>
      item.candidate_id === currentUser.id || item.jobs?.companies?.owner_id === currentUser.id,
    ));
  }, [user]);

  const loadMessages = useCallback(async (threadId, currentUser = user) => {
    if (!threadId || !currentUser || !supabase) return;
    ...
    setMessages(data || []);
    await supabase.rpc('mark_message_thread_read', { p_thread_id: threadId });
  }, [user]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    Promise.all([loadThreads(user), loadApplications(user)]).finally(() => setLoading(false));

    const timer = window.setInterval(() => {
      loadThreads(user);
      if (selectedId) loadMessages(selectedId, user);
    }, 10000);
  ...
  useEffect(() => {
    if (open && selectedId && user) loadMessages(selectedId, user);
  ...
  useEffect(() => {
    const installButtons = () => {
      ...
      if (nav && !nav.querySelector('[data-nzela-messages-v2]')) {
        ...
        button.type = 'button';
        button.dataset.nzelaMessagesV2 = 'true';
      ...
      if (actions && !actions.querySelector('[data-nzela-messages-mobile-v2]')) {
        ...
        const button = document.createElement('button');
        button.type = 'button';
        button.dataset.nzelaMessagesMobileV2 = 'true';
    ...
    const handleClick = async (event) => {
      const trigger = event.target instanceof Element
        ? event.target.closest('[data-nzela-messages-v2], [data-nzela-messages-mobile-v2]')
        : null;
  ...
  useEffect(() => {
    document.querySelectorAll('[data-nzela-messages-v2], [data-nzela-messages-mobile-v2]').forEach((button) => {
      ...
      if (!badge) {
        ...
        badge.dataset.messageBadgeV2 = 'true';
  ...
  const startConversation = async (application) => {
    ...
    const { data, error: createError } = await supabase.rpc('get_or_create_message_thread', {
  ...
  const sendMessage = async (event) => {
    ...
    if (!body || !selected || !user || !supabase || sending) return;
    ...
    if (sendError) {
      ...
      setDraft('');
      await Promise.all([loadMessages(selected.id, user), loadThreads(user)]);
    }
        ...
        <aside className={`${selected ? 'hidden md:flex' : 'flex'} w-full flex-col border-r border-slate-200 md:w-[390px]`}>
          <div className="border-b border-slate-200 p-5">
            <div className="flex items-center justify-between gap-3">
              ...
              <button onClick={() => setOpen(false)} className="secondary-icon-button" aria-label="Fermer">
                <X size={20} />
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {!loading && visibleThreads.map((thread) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full gap-3 border-b border-slate-100 p-4 text-left hover:bg-slate-50 ${selectedId === thread.id ? 'bg-blue-50' : 'bg-white'}`}
                >
                  <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-blue-700 text-sm font-bold text-white">
                    {initials(name)}
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="flex items-center justify-between gap-2">
                      <strong className="truncate text-sm text-slate-950">{name}</strong>
                      <small className="shrink-0 text-slate-500">{formatDate(thread.last_message_at)}</small>
                    </span>
            ...
            {!loading && threads.length === 0 && (
              <div className="p-5">
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-5 text-center">
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-600">Vos futurs échanges de recrutement apparaîtront ici.</p>
                </div>
                ...
                {applications.length > 0 && (
                  <div className="mt-5 space-y-2">
                    {applications.map((application) => (
                      ...
                      >
                        <strong className="block text-sm text-slate-950">{application.jobs?.title || 'Offre d’emploi'}</strong>
            ...
            {!loading && threads.length > 0 && visibleThreads.length === 0 && (
              <p className="p-6 text-center text-sm text-slate-500">Aucune conversation ne correspond à votre recherche.</p>
            )}
          </div>
        </aside>
          ...
          {selected ? (
            <>
              <header className="flex min-h-[82px] items-center gap-3 border-b border-slate-200 bg-white px-4 md:px-6">
                ...
                <span className="flex h-11 w-11 items-center justify-center rounded-lg bg-blue-700 font-bold text-white">
                  {initials(partner)}
                </span>
                <div className="min-w-0 flex-1">
                  <h3 className="truncate font-bold text-slate-950">{partner}</h3>
                  <p className="truncate text-sm text-slate-500">{selected.job_title} · {selected.company_name}</p>
                </div>
                <button onClick={() => setOpen(false)} className="secondary-icon-button hidden md:inline-flex" aria-label="Fermer">
                  <X size={20} />
                </button>
              </header>
              ...
              <div className="flex-1 overflow-y-auto px-4 py-6 md:px-8">
                <div className="mx-auto max-w-3xl space-y-4">
                  <div className="mx-auto max-w-xl rounded-xl border border-blue-200 bg-blue-50 p-4 text-center text-sm font-semibold text-slate-700">
                    Conversation liée à la candidature pour <strong>{selected.job_title}</strong>.
                  </div>
                      ...
                      <div key={message.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[82%] rounded-2xl px-4 py-3 shadow-sm ${mine ? 'rounded-br-md bg-blue-700 text-white' : 'rounded-bl-md border border-slate-200 bg-white text-slate-800'}`}>
                          ...
                          <p className={`mt-1 text-right text-[10px] ${mine ? 'text-blue-100' : 'text-slate-400'}`}>
                            {formatDate(message.created_at)}{mine && message.read_at ? ' · Lu' : ''}
                          </p>
                        </div>
                      </div>
                    );
                  ...
                  {messages.length === 0 && (
                    <p className="py-12 text-center text-sm text-slate-500">Écrivez le premier message de cette conversation.</p>
                  )}
                </div>
              </div>
              ...
              <form onSubmit={sendMessage} className="border-t border-slate-200 bg-white p-3 md:p-5">
                <div className="mx-auto flex max-w-3xl items-end gap-2">
                  <textarea
                    ...
                    className="min-h-12 max-h-36 flex-1 resize-none rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none focus:border-blue-700 focus:ring-2 focus:ring-blue-600"
                  />
                  <button type="submit" disabled={!draft.trim() || sending} className="primary-button h-12 w-12 px-0" aria-label="Envoyer">
                    <Send size={19} />
                  </button>
                </div>
            ...
            <div className="flex flex-1 items-center justify-center p-8 text-center">
              <div>
                <MessageSquare className="mx-auto text-blue-700" size={34} />
                <h3 className="mt-4 text-xl font-bold text-slate-950">Vos échanges de recrutement</h3>
                <p className="mt-2 text-sm text-slate-600">Sélectionnez une conversation.</p>
              </div>
            </div>
          )}
        </section>
