...
import { CheckCircle2, RefreshCw, ShieldCheck, X, XCircle } from 'lucide-react';
          ...
          <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 backdrop-blur">
            <div className="mx-auto flex max-w-5xl items-center justify-between gap-3 px-4 py-4 md:px-6">
              ...
              <div className="flex gap-2">
                <button type="button" onClick={load} className="flex h-11 w-11 items-center justify-center rounded-xl border border-slate-300 bg-white text-slate-700" aria-label="Actualiser">
                  <RefreshCw size={18} className={loading ? 'animate-spin' : ''} />
                </button>
                <button type="button" onClick={() => setOpen(false)} className="flex h-11 w-11 items-center justify-center rounded-xl bg-slate-950 text-white" aria-label="Fermer">
                  <X size={20} />
                </button>
              </div>
            </div>
              ...
              <article key={item.verification_id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                {item.status === 'pending' && (
                  <div className="mt-4 grid gap-2 sm:grid-cols-2">
                    <button type="button" onClick={() => review(item, 'approved')} className="flex min-h-11 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-black text-white hover:bg-emerald-700">
                      <CheckCircle2 size={18} /> Approuver
                    </button>
                    <button type="button" onClick={() => review(item, 'rejected')} className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-red-300 bg-white px-4 text-sm font-black text-red-700 hover:bg-red-50">
