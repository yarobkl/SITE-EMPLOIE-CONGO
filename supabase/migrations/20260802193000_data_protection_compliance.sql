...
create table if not exists public.consent_records (
  ...
  consent_type text not null check (consent_type in ('terms_acceptance','privacy_acknowledgement','application_data_transfer','marketing')),
  document_version text not null,
  source text not null default 'web',
  granted_at timestamptz not null default now(),
  revoked_at timestamptz,
  metadata jsonb not null default '{}'::jsonb,
  constraint consent_subject_required check (user_id is not null or subject_email is not null or tracking_number is not null)
);
...
create policy "users insert own consent records" on public.consent_records
  for insert to authenticated
  with check (user_id = (select auth.uid()) and char_length(document_version) between 1 and 40);
create policy "anonymous application consent can be recorded" on public.consent_records
  for insert to anon
  with check (
    user_id is null
...
create table if not exists public.privacy_requests (
  ...
  email text not null,
  tracking_number text,
  request_type text not null check (request_type in ('access','rectification','deletion','objection','portability','consent_withdrawal')),
  details text,
...
create policy "users create own privacy requests" on public.privacy_requests
  for insert to authenticated
  with check (user_id = (select auth.uid()) and char_length(email) between 3 and 320 and status = 'pending');
create policy "anonymous users create privacy requests" on public.privacy_requests
  for insert to anon
  with check (
    user_id is null and char_length(email) between 3 and 320 and status = 'pending'
    and (tracking_number is null or tracking_number ~ '^NZJ-[A-Z0-9-]{12,64}$')
  );
create policy "users read own privacy requests" on public.privacy_requests
  for select to authenticated using (user_id = (select auth.uid()));

-- Corrige les comparaisons réflexives de l'ancienne politique.
drop policy if exists "participants create message threads" on public.message_threads;
create policy "participants create message threads" on public.message_threads
  for insert to authenticated
  with check (
    ...
    and exists (
      ...
      where a.id = message_threads.application_id
        ...
        and (a.candidate_id = (select auth.uid()) or c.owner_id = (select auth.uid()))
    )
  );
